...
grep -Fq 'nginx -T' server/scripts/discover_nginx.py || fail "Effective nginx configuration discovery missing"
grep -Fq "probe_name='pride-probe-'" server/scripts/discover_nginx.py || fail "Live HTTPS webroot probe missing"
grep -Fq 'HTTPS probe matched' server/scripts/discover_nginx.py || fail "Ambiguous HTTPS mapping refusal missing"
grep -Fq "PB_REMOTE_URL_DEFAULT='https://mojoworks.xyz/labs/bank'" scripts/release_profile.sh || fail "Canonical Pride public URL default missing"
grep -Fq 'PB_PREFERRED_PUBLIC_URL=' scripts/deploy_backend.sh || fail "Backend deploy does not pass preferred canonical public URL"
grep -Fq -- '--preferred-base-url' server/scripts/discover_nginx.py || fail "Nginx discovery cannot select a proven canonical alias"
grep -Fq 'SELECTION=preferred-canonical-url' server/scripts/discover_nginx.py || fail "Preferred nginx alias selection audit output missing"
grep -Fq 'DISCOVERY_ARGS+=(--preferred-base-url "$PREFERRED_PUBLIC_URL")' server/scripts/bootstrap_ubuntu.sh || fail "Ubuntu bootstrap does not constrain nginx discovery to the canonical Pride URL"
grep -Fq 'PUBLIC_HEALTH_URL=' server/scripts/bootstrap_ubuntu.sh || fail "Public HTTPS API health verification missing"
grep -Fq 'rollback_nginx_mapping' server/scripts/bootstrap_ubuntu.sh || fail "nginx rollback helper missing"
grep -Fq 'STRIPE_WEBHOOK_ID' server/scripts/configure-stripe.js || fail "Stripe webhook endpoint id reconciliation missing"
grep -Fq 'Validated and normalized Pride server environment as data' server/scripts/bootstrap_ubuntu.sh || fail "Server env normalization guard missing"
grep -Fq 'python3 "$ENV_TOOL" exec "$ENV_FILE" -- node' server/scripts/bootstrap_ubuntu.sh || fail "Stripe provisioning must use safe env execution helper"
grep -Fq 'set-many-stdin' server/scripts/set_stripe_keys.sh || fail "Stripe credential writer must update the environment atomically"
grep -Fq 'STRIPE_EXPECTED_BUSINESS_NAME="WORKWORK.FUN LTD"' server/server.env.example || fail "Merchant name must be safely encoded in environment template"
grep -Fq 'def parse(path: Path)' server/scripts/env_file.py || fail "Safe server environment parser missing"
grep -Fq 'os.execvpe' server/scripts/env_file.py || fail "Safe environment process launcher missing"
if grep -Eq '(^|[;[:space:]])(source|\.)[[:space:]]+.*server\.env|set -a;[[:space:]]*\.[[:space:]]+"?\$ENV_FILE' server/scripts/*.sh scripts/*.sh; then fail "Pride server.env must never be sourced/executed as shell code"; fi
