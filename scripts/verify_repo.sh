...
grep -Fq 'cat "$migration"' server/scripts/bootstrap_ubuntu.sh || fail "PostgreSQL migration SQL must be streamed from the root process"
...
grep -Fq 'ALTER SCHEMA public OWNER TO pride_owner' server/migrations/004_schema_ownership.sql || fail "Pride schema ownership migration missing"
grep -Fq 'PB_DB_CHECK=' server/scripts/bootstrap_ubuntu.sh || fail "Pride database identity check missing before schema repair"
grep -Fq 'ALTER SCHEMA public OWNER TO pride_owner' server/scripts/bootstrap_ubuntu.sh || fail "Pride schema ownership bootstrap repair missing"
grep -Fq "printf 'BEGIN;\nSET LOCAL ROLE pride_owner;\n'" server/scripts/bootstrap_ubuntu.sh || fail "Migrations must run atomically under Pride owner role"
grep -Fq "INSERT INTO schema_migrations(name) VALUES" server/scripts/bootstrap_ubuntu.sh || fail "Migration bookkeeping must be inside atomic migration stream"
